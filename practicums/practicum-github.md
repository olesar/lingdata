...
На картинке пример репозитория у которого две ветки - `main` и `gh-pages`. Ветка `gh-pages` -- для GitHub pages, сервиса, который позволяет представлять содержание репозитория как сайта.  

<img src="../fig/github-branches.png" alt="branches" width="30%"/>  

Если вы не видите ветки `gh-pages`, нажмите на `main` и добавьте ветку `gh-pages` (`Create branch...`). Удалите ветку `main` и дальше работайте в единственной ветке, в `gh-pages`.    

В новой вкладке откройте ссылку `https://<my-account>.github.io/lingdata/`, чтобы увидеть страницу, представляющую содержание файла README.md в корне репозитория.  
...
Три обратных апострофа  (```) выделяют начало и конец блока текста, обычно кода программы
```
...
Некоторые html-теги требуют обязательно "обвязки" другими тегами. Например, теги списка `<li>` (*li*st) не будут работать без обвязки тегами `<ul>` (*u*nordered *l*ist) или `ol` (*o*rdered *l*ist) и т.п.
...
Если вы по ошибке форкнули не тот репозиторий, перейдите в его корень, выберите меню Settings и затем (в самом низу списка) `Delete this repository`. Будут стерты и сами файлы, и история их изменений.  
