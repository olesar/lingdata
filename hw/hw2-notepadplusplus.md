...
1.2 Замените в сохраненном файле все двойные сочетания `нн` на `н(н)` (то есть поставьте в скобки вторую букву _н_). Учитывайте регистр, меняйте строго строчные.
...
Напишите как можно более короткое регулярное выражение, которое найдёт все варианты написания названия ледника Эйяфьядлайёкюдль по-русски (Эяфьядлайёкюдль, Эйяфьяллайёкюдль, Эйяфьядлайёкудль...). Не забывайте про про разные формы слова. Но находиться должны только варианты этого слова (*Эйлер* и *ирландский дудль* находиться не должны). 

Результат представьте в файле eyjafjallajoekull.txt  

Мы хотим увидеть там на разных строчках:
- само регулярное выражение
- список всех вариантов, на которые вы ориентировались (они должны находиться вашим регулярным выражением)
- количество вариантов написания, которые покрывает ваше регулярное выражение (например, ко(т|д) находит 2 слова)
- объяснение, какими ресурсами вы пользовались для составления списка (если пользовались)
