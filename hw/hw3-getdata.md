...
***Не режьте видео! Работайте дальше с целиковым! (С тем метражом, который записали. Записали 20 минут -- загружайте в елан и в папку все 20 минут)***
...
* объем разметки на оценку - минимум 60 секунд
...
* имя файла с видео и имя файла разметки должны совпадать. Пример: - файлы `Cherepovets_pal2006.mp4` и `Cherepovets_pal2006.eaf`.
* загрузите файл разметки (он имеет расширение .eaf) на GitHub в папку `livecorpus` (в вашем репозитории `lingdata`)
* проверьте, что вы загружаете в единственную ветку `gh-pages` (ветка указана слева сверху над файлами вашего репозитрия)
...
Изучите [правила](http://www.spokencorpora.ru/showtranshelp.py) упрощённой (не полной, не минимальной) транскрипции и выполните расшировку минуты вашего фрагмента согласно им.   
...
* Тип - utterance  
Обязательно заполните поля "участник" (@aaa1988f), "разметчик" (латиницей - например, название вашего аккаунта на GitHub), язык по умолчанию (русский).  
```
...
* Кликните в слое на месте начала реплики - туда переместится бегунок - красная вертикальная полоса.  
...
* __Аббревиатуры__ записываются обычным образом (*ЦСКА*)
* __Числительные__ записываются словами (ср. *пятьсот восемнадцать*, *пять один восемь*) 
